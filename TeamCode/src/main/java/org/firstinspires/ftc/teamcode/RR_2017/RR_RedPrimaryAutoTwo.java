...
@Autonomous(name="Primary Red Two", group="3965")
public class RR_RedPrimaryAutoTwo extends RR_AutoBase {
    ...
    public void start() {
        ...
        stateMachine = new StateMachine(
                ...
                new SpinState("to spin around to grab", 0.0, new TimeTrans("spin around to grab", 1.0)),
                new SpinState("spin around to grab", 0.5, new ProgressTrans("to to blocks", 205.0)),
                new DriveState("to to blocks", 0.0, new TimeTrans("to blocks", 1.0)),
                new DriveState("to blocks", 0.5,
                        new TimeTrans("to spin back from grab", 8.0),
                        new BelowRangeTrans("pick up", 25.0)
                ),
                new PickUpBlockState("pick up", 0.0, //! TODO: Make this state acquire block
                        new TimeTrans("to spin back from grab", 4.0),
                        new BelowRangeTrans("to spin back from grab", 7.0)
                ),
                new SpinState("to spin back from grab", 0.0, new ProgressTrans("spin back from grab", 1.0)),
                new SpinState("spin back from grab", -0.5, new ProgressTrans("to drive back to box", 205.0)),
                new DriveState("to drive back to box", 0.0, new TimeTrans("drive back to box", 1.0)),
                new DriveState("drive back to box", 0.5, new TimeTrans("end", 4.0)),
                new WaitState("end")
